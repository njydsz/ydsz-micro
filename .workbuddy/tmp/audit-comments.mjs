...
const SKIP_DIRS = new Set([
  ...
  // 第三方离线回退资源（importmap 兜底，非本仓源码，详见 eslint.config.mjs）：
  // main/public/vendor 下为 cdn.jsdelivr.net 的 axios / vxe-pc-ui 等 ESM 副本，
  // 由 importmap 锁定版本升级时整包替换，不承担本仓编码规范，故排除统计。
  'vendor',
]);
...
function hasHeader(rawLines, file) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    // 自动生成文件头（/* auto-generated by ... — DO NOT EDIT */）视为合法模块头
    if (t.startsWith('/*') && /auto[\s-]?generated|do\s+not\s+edit|openapi-typescript/i.test(t)) {
      return { has: true, misplaced: false };
    }
...
/**
 * 判定文件是否为自动生成产物（导出注释豁免）。
 *
 * <p>在文件首部 25 行扫描标志：
 * <ul>
 *   <li>注释含 "auto-generated" / "automatically generated" / "autogenerated"</li>
 *   <li>注释含 "DO NOT EDIT" / "@openapi" 等机器生成占位</li>
 * </ul>
 *
 * <p>自动生成的 sdk/schema.d.ts / types-export.ts / .generated-archived/models.ts 等，
 * 手工维护的 JSDoc 在下次 pnpm gen:api 时会被覆盖，故不参与导出注释统计。
 */
function isAutoGenerated(rawLines) {
  const limit = Math.min(rawLines.length, 25);
  const AUTO_RE = /auto[\s-]?generated|automatically\s+generated|do\s+not\s+edit|@openapi/i;
  for (let i = 0; i < limit; i++) {
    const t = rawLines[i];
    if (t.trim() === '' || t.trim().startsWith('import')) continue;
    if (AUTO_RE.test(t)) return true;
    // 命中非注释代码行（import 已跳过）代表不属于自动生成头部
    if (CODE_START_RE.test(t.trim()) && !t.trim().startsWith('//') && !t.trim().startsWith('/*')) {
      return false;
    }
  }
  return false;
...
function analyzeFile(file) {
  ...
  // 自动生成文件（sdk/schema.dts / types-export / .generated-archived 等）不参与导出注释统计
  const autoGenerated = isAutoGenerated(rawLines);
  // 模块级 JSDoc/@fileoverview 覆盖全文件导出符号（符合 JSDoc "Module" 注释语义）
  const moduleHeaderCoversExports = headerInfo.has && !autoGenerated;

  // 抽取 <script> 段 + 剔除模板字符串内容，避免模板字面量中的 export 行被误判为真实导出
  const code = autoGenerated ? '' : stripTemplateLiterals(extractScript(raw, file));
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    exportTotal++;
    // 模块级 JSDoc 覆盖全文件：文件有合法头注释时所有导出均视为已注释
    if (moduleHeaderCoversExports) {
      exportDoc++;
      continue;
    }
    let j = i - 1;
