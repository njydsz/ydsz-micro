 ...
 * 面板（panel.ts）独立运行在此 sandbox 中，通过 chrome.runtime 与 background 通信。
 *
 * 职责：
 ...
 *
 * 消息流（简化）：
 *   background ──► devtools.ts runtime.onMessage ──► postMessage ──► inspectedWindow.eval ──► panel.ts
 *
 * 注意：DevTools Panel 与 background 隔离，消息必须经过 inspectedWindow.eval 转发。
 *
 * @path chrome/devtools/devtools.ts
 * @author ydsz-team
 * @since 4.0.0
 */
...
chrome.devtools.panels.create('Micro Kernel', '', 'devtools/panel.html', (panel) => {
  ...
  chrome.runtime.onMessage.addListener((msg: { target?: string }) => {
    if (msg.target !== 'devtools') return;
    // 通过 inspectedWindow.eval 转发到页面（因 DevTools Panel 与 background 隔离）
    const jsonStr = JSON.stringify(msg).replace(/\\/g, '\\\\').replace(/'/g, "\\'");
    const code = `window.postMessage({ source: 'ext-bg', detail: ${jsonStr} }, '*')`;
    void chrome.devtools.inspectedWindow.eval(code).catch(() => {
      /* inspected page may have been navigated away */
    });
  });
