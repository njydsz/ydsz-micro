/**
 * VXe YdTable 在项目中的完整初始化入口，按需注册组件并同步主题 / 语言。
 *
 ...
 *
 * @remarks
 * 由于 `useYdForm` 依赖运行时注册的 vxe-table 组件，此处先声明可变的
 * 导出占位，待 {@link initVxeTable} 完成组件注册后再赋值为真正的实现，
 * 供业务侧统一从本模块导入，避免时序问题。
 */
export let useTableForm: typeof useYdForm;
...
export function setupYDSZVxeTable(setupOptions: SetupVxeTable) {
  const { configVxeTable, useYdForm } = setupOptions;

  initVxeTable();
  useTableForm = useYdForm;
