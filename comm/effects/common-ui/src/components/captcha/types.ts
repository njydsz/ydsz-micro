/**
 * 点选验证码组件的类型契约：交互配置、校验回调载荷与对外暴露的方法。
 *
 * 与组件实现分开存放，是因为验证码需要被登录/注册等多个认证表单复用，
 * 各表单只需按这里的类型处理「用户点了哪些点、校验结果如何」，
 * 不必引入组件本体，也便于替换不同的验证码实现。
 *
 * 坐标相关字段统一以**验证码图片左上角**为原点、单位 px，可直接提交后端比对。
 *
