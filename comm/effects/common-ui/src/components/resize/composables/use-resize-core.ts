 ...
 * 约束/限制纯计算函数在 use-resize-limits.ts（此处 re-export 保持向后兼容）。
...
import { rectCorrectionByLimit } from './use-resize-utils';
import {
  calcDragLimitation,
  calcResizeLimits,
  rectCorrectionByAspectRatio,
} from './use-resize-limits';

export { calcDragLimitation, calcResizeLimits, rectCorrectionByAspectRatio };
