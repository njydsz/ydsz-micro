...
import { MessageCircle, Send, CheckCircle2, AlertTriangle, X } from '@remi/icons';
...
import { sentryCaptureError, isSentryInitialized } from '@remi/monitor/sentry';
import { sentrySetUser } from '@remi/monitor/sentry';
...
const t = (key: string): string => {
  const messages: Record<string, string> = {
    'feedback.title': '报告问题',
    'feedback.description': '您遇到了一个问题，是否愿意帮助我们改进？您的反馈将发送给开发团队。',
...
async function submitFeedback(): Promise<void> {
  ...
  try {
    ...
    console.error('[ErrorFeedback] Failed to submit:', err);
...
<template>
  <div class="error-feedback">
    ...
    <Dialog :open="isOpen" @update:open="(val: boolean) => (isOpen = val)">
      ...
      <DialogContent class="sm:max-w-[500px]" role="dialog" aria-modal="true" :aria-label="t('feedback.title')">
        <DialogHeader>
        ...
        <!-- 错误信息摘要 -->
        <div
          v-if="errorMessage || props.errorMessage"
          class="rounded-md bg-destructive/10 p-3 text-sm"
          role="alert"
          aria-live="polite"
        >
          <p class="font-medium text-destructive">{{ t('feedback.includeErrorInfo') }}：</p>
        ...
        <div v-if="submitStatus !== 'success'" class="grid gap-4 py-4">
          ...
          <div class="grid gap-2">
            ...
            <Textarea
              ...
              :disabled="submitStatus === 'submitting'"
            />
            <p v-if="errorMessage && submitStatus === 'error'" class="text-xs text-destructive">
              {{ errorMessage }}
            </p>
          </div>
        </div>
        ...
        >
          ...
          <p class="text-lg font-medium text-green-600">{{ t('feedback.success') }}</p>
        </div>
        ...
        <DialogFooter v-if="submitStatus !== 'success'">
          ...
          <REMIButton
            ...
            class="gap-2"
          >
            <Send v-if="submitStatus !== 'submitting'" class="size-4" aria-hidden="true" />
            <span
              v-else
