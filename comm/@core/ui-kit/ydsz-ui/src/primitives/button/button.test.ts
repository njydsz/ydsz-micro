/**
 * YdButton 组件测试 — 验证变体渲染、as 属性与可访问性基础
 *
...
describe('YdButton', () => {
  ...
  it('variant="destructive" 应包含对应 bg 类名', () => {
    const wrapper = mount(YdButton, { props: { variant: 'destructive' } });
    const classList = wrapper.classes();
  ...
  it('variant="outline" 应包含 border 类名', () => {
    const wrapper = mount(YdButton, { props: { variant: 'outline' } });
    const classList = wrapper.classes();
    expect(classList).toContain('border');
  });

  it('size="sm" 应包含对应尺寸类名', () => {
    const wrapper = mount(YdButton, { props: { size: 'sm' } });
    const classList = wrapper.classes();
    // sm 对应 h-8
  ...
  it('size="lg" 应包含对应大尺寸类名', () => {
    const wrapper = mount(YdButton, { props: { size: 'lg' } });
    const classList = wrapper.classes();
    // lg 对应 h-10
