<!--
 * VSelect 触发器：封装 SelectTrigger + SelectValue，统一触发器外观与可清选择。
 *
 * 设计目标：
 *  - 与原生 SelectTrigger 保持一致的视觉（圆角、边框、focus ring、disabled 置灰）；
 *  - 右侧集成清除按钮，仅在已选项存在时显示；
 *  - 遵循 @see YdSelect 现有样式约定。
 *
 * 注意：Trigger 本身不持有值模型，需由父级 @see VSelect 显式传入 hasValue 控制清除按钮显隐。
 *
 * @path comm\@core\ui-kit\shadcn-ui\src\ui\select\VSelectTrigger.vue
 * @author ydsz-team
 * @since 1.0.0
-->
