 ...
 * 1. 禁止子应用直接 import axios（应从 @remi/shared-auth 导入 requestClient）
 * 2. 禁止子应用封装通用 CRUD 工厂（createCrudApi / CrudApi 等）
 * 3. 禁止子应用直接实现 auth 逻辑（应从 @remi/shared-auth 导入）
  ...
  rules: {
    ...
    'no-restricted-syntax': [
      'error',
      {
        ...
        message: '禁止直接 import axios，请从 @remi/shared-auth 导入 requestClient',
      },
      {
        ...
        message:
          '禁止子应用直接实现认证逻辑，请从 @remi/shared-auth 导入对应方法',
      },
    ],
    ...
    'no-restricted-imports': [
      'error',
      {
        // 子应用不能从 @remi/shared-auth 以外的包导入认证相关 API
        patterns: [
          {
            group: ['#/api/core/auth'],
            message: '请从 @remi/shared-auth 直接导入，而非子应用本地副本',
          },
          {
            group: ['#/api/core/user'],
            message: '请从 @remi/shared-auth 直接导入，而非子应用本地副本',
          ...
          {
            group: ['#/api/request'],
            message: '请从 @remi/shared-auth 导入 requestClient，而非子应用本地副本',
          },
          {
            group: ['#/store/auth'],
            message: '请从 @remi/shared-auth 导入 createSharedAuthStore，而非子应用本地副本',
          },
          {
            group: ['@remi/shared-api'],
            message:
              '@remi/shared-api 已删除，禁止封装通用 CRUD 工厂。每个 API 文件应显式定义各个 API 方法（使用 requestClient.get/post/put/delete）',
          },
        ],
      },
    ],
      ...
      rules: {
        'no-restricted-syntax': [
          'error',
          {
            ...
            message:
              '禁止子应用直接创建 authStore，请从 @remi/shared-auth 导入 createSharedAuthStore',
          },
        ],
      },
    },
      ...
      rules: {
        'no-restricted-syntax': [
          'warn',
          {
            ...
            message:
              '禁止直接写 window 属性（微前端沙箱边界）。请使用模块级变量或 Pinia store 管理状态；如确需全局共享，请通过 @remi/micro-runtime 的 globalStateAPI 通信',
          },
          {
