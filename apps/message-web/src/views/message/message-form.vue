...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateMessageApi(formData as MessageApi.MessageDTO); ElMessage.success('更新成功'); }
      else { await createMessageApi(formData as MessageApi.MessageDTO); ElMessage.success('创建成功'); }
