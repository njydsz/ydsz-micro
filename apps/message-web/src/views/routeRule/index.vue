...
/**
 * 消息路由（列表页）
 * <p>消费后端契约 RouteRuleController（src/api/routeRule.ts，auto-generated by bash/gen-contract.py）：
 * page() 分页查询路由规则，行操作「编辑」打开 routeRule-form.vue、「删除」调 deleteApi({id})。
...
const gridOptions: VxeTableGridOptions<MsgRouteRuleVO> = {
  ...
  formConfig: {
    enabled: true,
    items: [
      { field: 'ruleName', title: '规则名称', itemRender: { name: 'Input', props: { placeholder: '规则名称' } } },
      { field: 'ruleCode', title: '规则编码', itemRender: { name: 'Input', props: { placeholder: '规则编码' } } },
...
async function handleDelete(row: MsgRouteRuleVO) {
  if (!row.id) return;
  try {
    await ElMessageBox.confirm(`确定删除路由规则「${row.ruleName}」吗？`, '删除确认', { type: 'warning' });
