 ...
 *
 * <p>通知发送等场景需要「按关键字搜索用户」以支撑 @提及选择器：提交时把选中用户 ID
 * 写入 NotificationSendDTO.mentionUserIds（逗号分隔的字符串）。
 * 后端能力位于 userinfo 服务（GET /api/v1/userinfo/search），经统一网关调用；
 * 本文件为手写封装（非 bash/gen-contract.py 生成，勿覆盖）。
 *
 * @path apps/message-web/src/api/userSearch.ts
 * @author ydsz-team
 * @since 4.2.0
 */
...
export interface UserSearchHit {
  /** 用户 ID（选中后作为 mention/receiver 关联值） */
  id?: string;
  /** 显示名称 */
  title?: string;
  /** 登录名等辅助信息 */
