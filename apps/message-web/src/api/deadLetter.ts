/**
 * 死信队列 API 模块（前端）
 * <p>封装死信队列（{@code ydsz_message_dead_letter}）接口，对应后端 {@code /api/v1/message/dead-letter/*} 端点。
...
export function getDeadLetterPageApi(params: DeadLetterApi.DeadLetterPageQuery) {
  ...
  }>(`/api/v1/message/dead-letter/page`, { params });
}

/** 查询全部列表 */
export function getDeadLetterListApi() {
  return requestClient.get<DeadLetterApi.DeadLetterVO[]>(`/api/v1/message/dead-letter/list`);
}

/** 根据 ID 查询 */
export function getDeadLetterByIdApi(id: string) {
  return requestClient.get<DeadLetterApi.DeadLetterVO>(`/api/v1/message/dead-letter/${id}`);
}

/** 创建 */
export function createDeadLetterApi(data: DeadLetterApi.DeadLetterDTO) {
  return requestClient.post<string>(`/api/v1/message/dead-letter`, data);
}

/** 更新 */
export function updateDeadLetterApi(data: DeadLetterApi.DeadLetterDTO) {
  return requestClient.put<boolean>(`/api/v1/message/dead-letter`, data);
}

/** 删除 */
export function deleteDeadLetterApi(id: string) {
  return requestClient.delete<boolean>(`/api/v1/message/dead-letter/${id}`);
}
