/** auto-generated by bash/gen-contract.py — DO NOT EDIT */
// @data-file 纯类型 API 封装生成件（不计入行数/复杂度规则）
/**
 * SecurityAlertController API 封装（auto-generated by bash/gen-contract.py）
 *
 * <p>对应后端 {@code SecurityAlertController}，共 5 个端点。
 * <p>路径规范: /api/userinfo/**（kebab-case），成功码统一为 code === 'A00000'。
 *
 * @author ydsz-team
 * @auto-generated 请勿手动修改；后端契约变更后执行 {@code python bash/gen-contract.py userinfo} 重新生成
 * @since 1.0.0
 */
import { requestClient } from '#/api/request';
import type { PageResponse } from './models';
import type { SecurityAlert, SecurityAlertPageQuery } from './models';

/**
 * pageAlerts: GET /api/admin/security/alerts
 */
export function pageAlerts(params: {
    query?: SecurityAlertPageQuery;
  }): Promise<PageResponse<SecurityAlert[]>> {
  return requestClient.get<PageResponse<SecurityAlert[]>>(`/api/admin/security/alerts`, { params });
}

/**
 * getPendingAlerts: GET /api/admin/security/alerts/pending
 */
export function getPendingAlerts(params: {
    riskLevel?: string;
    limit?: number;
  }): Promise<SecurityAlert[]> {
  return requestClient.get<SecurityAlert[]>(`/api/admin/security/alerts/pending`, { params });
}

/**
 * acknowledgeAlert: PUT /api/admin/security/alerts/{id}/acknowledge
 */
export function acknowledgeAlert({ id }: {
    id: string;
  }, params: {
    note?: string;
  }): Promise<boolean> {
  return requestClient.put<boolean>(`/api/admin/security/alerts/${id}/acknowledge`, { params });
}

/**
 * resolveAlert: PUT /api/admin/security/alerts/{id}/resolve
 */
export function resolveAlert({ id }: {
    id: string;
  }, params: {
    note?: string;
  }): Promise<boolean> {
  return requestClient.put<boolean>(`/api/admin/security/alerts/${id}/resolve`, { params });
}

/**
 * ignoreAlert: PUT /api/admin/security/alerts/{id}/ignore
 */
export function ignoreAlert({ id }: {
    id: string;
  }, params: {
    note?: string;
  }): Promise<boolean> {
  return requestClient.put<boolean>(`/api/admin/security/alerts/${id}/ignore`, { params });
}
