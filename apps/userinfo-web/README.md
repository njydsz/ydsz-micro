...
> REMI PMIS 用户中心子应用，提供用户、角色、菜单、部门、公司、岗位与语言管理（对应后端：remi-userinfo，路由前缀：/remi-user）
...
|---|---|
| **类型** | 微前端子应用 |
| **包名** | @remi/userinfo-web |
| **对应后端服务** | remi-userinfo |
| **前端端口** | 5601 |
| **后端端口** | 9002 |
| **路由前缀** | /remi-user |
| **版本** | 1.0.0 |
...
子应用需配合主应用（main-web，5600）一起访问，主应用通过路径前缀 `/remi-user` 激活本应用。
...
主应用通过 `/remi-user` 前缀激活本应用，内部路由表（`src/router/routes/modules/system.ts`）分为两组：
...
API 模块位于 `src/api/`，使用 `@remi/request` 的 `requestClient`，对应后端 `/api/v1/*` 端点：
...
其他 API 模块：`role.ts`（角色）、`menu.ts`（菜单）、`dept.ts`（部门）、`post.ts`（岗位）、`company.ts`（公司）、`language.ts`（语言）。认证与菜单 API 复用 `src/api/core/`（`@remi/shared-auth`）。
