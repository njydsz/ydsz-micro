...
| 审计日志     | 规则执行轨迹与审计查询             | `views/audit-log/`  |
...
| `src/views/audit-log/index.vue`  | 审计日志，查询规则执行轨迹与命中结果      |
...
其他 API 模块：`dsl.ts`（DSL 管理）、`variable.ts`（规则变量）、`cep.ts`（CEP 复杂事件）、`breakpoint.ts`（断点调试）、`auditLog.ts`（审计日志）。认证与菜单 API 复用
