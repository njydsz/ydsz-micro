...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateDslApi(formData as DslApi.DslDTO); ElMessage.success('更新成功'); }
      else { await createDslApi(formData as DslApi.DslDTO); ElMessage.success('创建成功'); }
