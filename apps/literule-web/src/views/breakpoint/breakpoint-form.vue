...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateBreakpointApi(formData as BreakpointApi.BreakpointDTO); ElMessage.success('更新成功'); }
      else { await createBreakpointApi(formData as BreakpointApi.BreakpointDTO); ElMessage.success('创建成功'); }
