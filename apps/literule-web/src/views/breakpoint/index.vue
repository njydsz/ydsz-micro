...
const gridOptions: VxeGridProps<DebugRow> = {
  columns: [
    ...
    { field: 'hitCount', title: '命中次数', width: 90 },
    {
      ...
      slots: { default: ({ row }) => h(ElTag, { type: 'success' }, () => String(row.status ?? '-')) },
    },
    { field: 'createdAt', title: '创建时间', width: 160 },
    {
      field: 'action', title: '操作', width: 90, fixed: 'right',
      slots: { default: ({ row }) => h(ElButton, { size: 'small', link: true, type: 'danger', onClick: () => handleRemoveBreakpoint(row) }, () => '删除') },
    },
  ],
  height: 'auto',
  proxyConfig: {
...
async function handleCreateSession() {
  try {
    const { value } = await ElMessageBox.prompt('请输入会话关联的规则编码（可留空）', '创建调试会话', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      inputPlaceholder: 'ruleCode',
    });
  ...
  <Page auto-content-height>
    <div class="flex flex-col gap-3 p-4">
      <Grid table-title="断点列表">
        <template #toolbar-tools><ElButton type="primary" @click="handleAddBreakpoint">新增断点</ElButton></template>
      </Grid>
      <div class="rounded border border-gray-200 bg-white p-3">
        <div class="mb-2 flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="text-sm font-medium">调试会话</span>
            <span v-if="selectedSessionId" class="text-xs text-gray-500">当前会话：{{ selectedSessionId }}</span>
          </div>
        ...
        <ElTable :data="sessions" border size="small" class="mb-2">
          ...
          <ElTableColumn label="操作" width="160" fixed="right">
            <template #default="{ row }">
              <ElButton link type="primary" size="small" @click="handleSelectSession(row)">选择</ElButton>
              <ElButton link type="danger" size="small" @click="handleTerminateSession(row)">结束</ElButton>
            </template>
          </ElTableColumn>
        </ElTable>
        <div class="flex items-start gap-3">
          <ElInput
            ...
            class="flex-1"
          />
          <ElButton type="primary" :disabled="!selectedSessionId" @click="handleSubmitCommand">提交命令</ElButton>
        </div>
        <pre v-if="commandResult" class="mt-2 overflow-auto rounded border border-gray-300 bg-gray-50 p-3 text-xs">{{ commandResult }}</pre>
      </div>
    </div>
  </Page>
</template>