/** auto-generated by bash/gen-contract.py — DO NOT EDIT */
// @data-file 纯类型 API 封装生成件（不计入行数/复杂度规则）
/**
 * RuleImportExportController API 封装（auto-generated by bash/gen-contract.py）
 *
 * <p>对应后端 {@code RuleImportExportController}，共 3 个端点。
 * <p>路径规范: /api/v1/literule/**（kebab-case），成功码统一为 code === 'A00000'。
 *
 * @author ydsz-team
 * @path apps\literule-web\src\api\ruleImportExport.ts
 * @auto-generated 请勿手动修改；后端契约变更后执行 {@code python bash/gen-contract.py literule} 重新生成
 * @since 1.0.0
 */
import { requestClient } from '#/api/request';
import type { RuleImportDTO } from './models';

/**
 * exportRules: GET /api/v1/literule/rules/export
 */
export function exportRules(): Promise<Record<string, Record<string, unknown>>> {
  return requestClient.get<Record<string, Record<string, unknown>>>(`/api/v1/literule/rules/export`);
}

/**
 * exportRulesAsYaml: GET /api/v1/literule/rules/export.yaml
 */
export function exportRulesAsYaml(): Promise<string> {
  return requestClient.get<string>(`/api/v1/literule/rules/export.yaml`);
}

/**
 * importRules: POST /api/v1/literule/rules/import
 */
export function importRules(data: RuleImportDTO): Promise<Record<string, Record<string, unknown>>> {
  return requestClient.post<Record<string, Record<string, unknown>>>(`/api/v1/literule/rules/import`, data);
}
