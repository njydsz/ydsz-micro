...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateJobLogApi(formData as JobLogApi.JobLogDTO); ElMessage.success('更新成功'); }
      else { await createJobLogApi(formData as JobLogApi.JobLogDTO); ElMessage.success('创建成功'); }
