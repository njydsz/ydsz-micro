/** auto-generated by bash/gen-contract.py — DO NOT EDIT */
/**
 * TaskTopologyController API 封装（auto-generated by bash/gen-contract.py）
 *
 * <p>对应后端 {@code TaskTopologyController}，共 3 个端点。
 * <p>路径规范: /api/cronjob/**（kebab-case），成功码统一为 code === 'A00000'。
 *
 * @author ydsz-team
 * @auto-generated 请勿手动修改；后端契约变更后执行 {@code python bash/gen-contract.py cronjob} 重新生成
 * @since 1.0.0
 */
import { requestClient } from '#/api/request';
import type { PageResponse } from './base';
import type { JobLogVO } from './models';

/**
 * getDagInstanceTopology: GET /api/cronjob/topology/dagInstance/{dagInstanceId}
 */
export function getDagInstanceTopology(path: {
    dagInstanceId: string;
  }): Promise<unknown> {
  return requestClient.get<unknown>(`/api/cronjob/topology/dagInstance/${dagInstanceId}`);
}

/**
 * getDagInstanceCytoscape: GET /api/cronjob/topology/dagInstance/{dagInstanceId}/cytoscape
 */
export function getDagInstanceCytoscape(path: {
    dagInstanceId: string;
  }): Promise<unknown> {
  return requestClient.get<unknown>(`/api/cronjob/topology/dagInstance/${dagInstanceId}/cytoscape`);
}

/**
 * getJobExecutionHistory: GET /api/cronjob/topology/jobHistory/{jobKey}
 */
export function getJobExecutionHistory(path: {
    jobKey: string;
  }): Promise<JobLogVO[]> {
  return requestClient.get<JobLogVO[]>(`/api/cronjob/topology/jobHistory/${jobKey}`);
}
