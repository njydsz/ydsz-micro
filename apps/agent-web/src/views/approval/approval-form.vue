...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateApprovalApi(formData as ApprovalApi.ApprovalDTO); ElMessage.success('更新成功'); }
      else { await createApprovalApi(formData as ApprovalApi.ApprovalDTO); ElMessage.success('创建成功'); }
