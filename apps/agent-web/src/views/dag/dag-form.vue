...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateDagApi(formData as DagApi.DagDTO); ElMessage.success('更新成功'); }
      else { await createDagApi(formData as DagApi.DagDTO); ElMessage.success('创建成功'); }
