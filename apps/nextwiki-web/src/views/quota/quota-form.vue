...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateQuotaApi(formData as QuotaApi.QuotaDTO); ElMessage.success('更新成功'); }
      else { await createQuotaApi(formData as QuotaApi.QuotaDTO); ElMessage.success('创建成功'); }
