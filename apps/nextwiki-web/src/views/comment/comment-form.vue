...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateCommentApi(formData as CommentApi.CommentDTO); ElMessage.success('更新成功'); }
      else { await createCommentApi(formData as CommentApi.CommentDTO); ElMessage.success('创建成功'); }
