...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateFileApi(formData as FileApi.FileDTO); ElMessage.success('更新成功'); }
      else { await createFileApi(formData as FileApi.FileDTO); ElMessage.success('创建成功'); }
