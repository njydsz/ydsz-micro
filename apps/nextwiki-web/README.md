...
|---|---|
| **类型** | 微前端子应用 |
| **包名** | @remi/nextwiki-web |
| **对应后端服务** | remi-nextwiki |
| **前端端口** | 5607 |
| **后端端口** | 9003 |
| **路由前缀** | /remi-wiki |
| **版本** | 1.0.0 |
...
子应用需配合主应用（main-web，5600）一起访问，主应用通过路径前缀 `/remi-wiki` 激活本应用。
...
主应用通过 `/remi-wiki` 前缀激活本应用，内部路由表（`src/router/routes/modules/nextwiki.ts`）分为四组：
...
API 模块位于 `src/api/`，使用 `@remi/request` 的 `requestClient`，对应后端 `/api/v1/nextwiki/*` 端点：
...
其他 API 模块：`comment.ts`（文件评论）、`share.ts`（分享管理）、`tag.ts`（标签管理）、`quota.ts`（配额管理）。认证与菜单 API 复用 `src/api/core/`（`@remi/shared-auth`）。
