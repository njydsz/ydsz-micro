...
export function pageByCursor(params: {
  ...
  return requestClient.get<PageResponse<ConfigVO>>(`/api/v1/config/cursor`, { params });
}
...
export function batchSave(data: ConfigBatchDTO): Promise<Record<string, Record<string, unknown>>> {
  return requestClient.post<Record<string, Record<string, unknown>>>(`/api/v1/config/batch`, data);
}
...
export function importConfigs(params: {
  ...
  return requestClient.post<ImportResultVO>(`/api/v1/config/import`, { params });
}
