...
// TODO: [shadcn-ui migration pending] ElForm/ElFormItem/ElRadio/ElRadioGroup/ElInputNumber 暂无 shadcn FormField 替代方案，待 UI Kit 表单方案统一后迁移
  ...
  onConfirm: async () => {
    ...
    try {
      if (isEdit.value) {
        ...
        showToast.success(t('configUpdateSuccess'));
      } else {
        await save(formData);
        showToast.success(t('configCreateSuccess'));
      }
      emit('success');
      modalApi.close();
    } finally {
