...
子应用需配合主应用（main-web，5600）一起访问，主应用通过路径前缀 `/remi-sys` 激活本应用。
...
主应用通过 `/remi-sys` 前缀激活本应用，内部路由表（`src/router/routes/modules/system.ts`）统一归在「系统管理」组下：
...
其他 API 模块：`dictType.ts`（字典类型）、`dictItem.ts`（字典项）、`variable.ts`（系统变量）、`app.ts`（应用注册）。认证与菜单 API 复用 `src/api/core/`（`@remi/shared-auth`）。
...
4. **认证复用**：本应用依赖 `@remi/shared-auth`，不重复实现登录，token 通过 `useTokenStore()` 从 SecureLS 读取，由 `@remi/shared-auth` 统一管理。
