...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    modalApi.lock();
    try {
      if (isEdit.value) { await updateDelegateApi(formData as DelegateApi.DelegateDTO); ElMessage.success('更新成功'); }
      else { await createDelegateApi(formData as DelegateApi.DelegateDTO); ElMessage.success('创建成功'); }
